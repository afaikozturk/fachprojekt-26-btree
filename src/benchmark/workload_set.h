...
public:
    ...
    {
        build(insert_workload_file, mixed_workload_file);
    }
    NumericWorkloadSet(NumericWorkloadSet &&) noexcept = default;
    ~NumericWorkloadSet() = default;

    void build(const std::string &insert_workload_file, const std::string &mixed_workload_file);

    [[nodiscard]] const std::vector<NumericTuple> &insert() const noexcept { return _data_sets[0]; }
    ...
    explicit operator bool() const { return insert().empty() == false || mixed().empty() == false; }
